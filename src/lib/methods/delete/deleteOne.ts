...
export default async function findOne(spesicifData: {
    ...
    if (querySnapshot.size > 1) this.eventTrigger("warning", `deleteOne() returned more than one document. Only the first document will be deleted. (collection: ${this.collection})`)
    let myDoc = querySnapshot.docs[0];
    await deleteDoc(myDoc.ref);

    this.eventTrigger("delete", myDoc.data());
